        ...
        <template v-for="pip, index in data.pips" :key="pip.index">
          <div class="pipBar" :style="boardData.pipBlockStyles[(config.isTop ? 'top' : 'bottom') + (config.isLeft ? 'Left' : 'Right')][index]">
            <div class="pipBarWrapper">
              ...
              <div class="stones">
                <template v-if="boardData.pips[pip.index].count > 0">
                  <template v-for="(n,index) in boardData.pips[pip.index].count" :key="index">
                    <!-- index < 5 -->
                    <template v-if="index < 5">
                      <Stones :isTop="config.isTop" :pip="pip" :index="index" :stoneLayer="0" />
                    </template>
                    <!-- index < 10 -->
                    <template v-else-if="index < 10">
                      <Stones :index="index" :stoneLayer="1" :pip="pip" :config="config.pipBlocks[boardData.boardDirection].tl" />
                    </template>
                    <!-- index < 15 -->
                    <template v-else>
                      <Stones :index="index" :stoneLayer="2" :pip="pip" :config="config.pipBlocks[boardData.boardDirection].tl" />
                    </template>             
                  </template>
                </template>
              </div>
